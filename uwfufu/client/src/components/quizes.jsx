            ...
            <div className="d-flex justify-content-around flex-wrap flex-start">
                {quizzes.map(quiz => (
                    <div className="d-flex flex-column align-items-center border my-2" style={{width: "38vw", height: "45vh", borderRadius: "10px"}}>
                        <img src={`http://${config.host + ":" + config.serverPort}/` + quiz.title_img} alt={quiz.title} className="p-1" style={{width: "35vw", height: "20vh", objectFit: "cover"}}/>
                        <p>{quiz.title}</p>
                        <p>{quiz.status}</p>
                        <p>{quiz.owner_id == user.id ? "Mój Quiz" : null}</p>
                        <Button className="primary" href={`http://${config.host + ":" + config.port}/` + quiz.id} style={{width: "35vw"}}>Graj</Button>
                    </div>
                ))}
